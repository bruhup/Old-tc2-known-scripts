    ...
    task.spawn(function()
        ...
        for i,v in Tags do
            if i == Player:GetAttribute("GroupRank") then
                notiflib.NormalNotification({
                    Title = "⚠ High Group Rank Player Detected ⚠",
                    Description = "⚠ '".. Player.DisplayName .. " (@".. Player.Name ..")' is ranked '".. v .."' in the Dorcus Digital Group.",
                    ImagesID = "http://www.roblox.com/asset/?id=7733964719", 
                    SoundID = "rbxassetid://8784885431", -- make sure the sound id is authorized by roblox
                    Duration = 25
                })
            end
        end
...
local gamename = game.PlaceId == 328028363 and "Typical Colors 2" or "Universal"

if _G.library then
    _G.library:Unload()
end

if _G.Watermarke then
    _G.Watermarke()
end
...
local CS = {
    ...
    BibulusCS = {
        "Bibulus on top",
        "Alsike : Sponsored by Bibulus software",
        ".gg/eM3S4BsxY4",
        "Sponsored by the Bibulus Team",
        "Aren't you a little too young to be losing? Use Bibulus.club!",
        "#catbotinvasionpart2?",
        "Bibulus : Catbot partnership",
        "BeanBoozled WEGA Ban Ban2 there's no way this can crash studio LOL!"
    },
    AlsikeKS = {
    	"me > you",
    	"Tbh you suck...", 
    ...
    BibulusKS = {
        "Looks like i BeanBoozled you...",
        "Aren't you a little too young to be losing?",
        "You're going to be a good CatBot.",
        "I'm Sponsored by the Bibulus Team.",
        "Projectile Aimbotted.",
        "Hitscan Aimbotted.",
        "Melee Aimbotted.",
    },
    ToxicKS = {
        "EZ EZ EZ",
        "Lol bro is trash",
        "Can bro even kill somebody?",
        "I would say ur aim is cancer but cancer actually kills ppl.",
        "Bro is fr going to aim better at the Leave button than at me",
        "Here on Cheater Police: We have this loser dying to ME LOL!",
        "Even if bro has aimbot he would still die LOL",
        "Bro is rlly targetting me XDD",
    },
    MrSandmanKS = {
        "Tell your hitbox to leave my gun alone.",
        "Night-night!",
        "This kill's mine, dude. You ain't never gonna take it.",
        "Put you to bed, because it's waaay past your bedtime.",
...
local library = {design = _G.design == "kali" and "kali" or "Alsike", tabs = {}, draggable = true, flags = {}, title = "alsike", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "alsike", configgame = gamename, fileext = ".al"}
_G.library = library
...
library.createBind = function(option, parent)
    ...
    local bindinput = library:Create(option.sub and "TextButton" or "TextLabel", {
        Position = UDim2.new(1, -6 - (option.subpos or 0), option.IsButton and -0.15 or 0, option.sub and 3 or 4),
...
library.createList = function(option, parent)
    ...
    table.insert(library.theme, option.content)
...
function library:Init()
    ...
    self.thiswasveryconfusing = self:Create("TextLabel", {
...
local function RenderChams2(part, color, trans, reflectance, material)
	part.Material = material == "ForceField" and "ForceField" or material == "Neon" and "Neon" or material == "Plastic" and "SmoothPlastic" or "Glass"
	--part.VertexColor = V3(color.R, color.G, color.B)
	part.Color = color
	part.Transparency =  1 - trans
	part.Reflectance = reflectance / 10 
	if material == "Plastic" or material == "Glass" then 
        ...
        for i,v in part:GetChildren() do
            if v.Name == "FuckingTexture" then
                part.TextureID = part:WaitForChild("FuckingTexture").Value 
            end
        end
	end 
end
...
library.RageTab = library:AddTab("Rage")
library.LegitTab = library:AddTab("Legit")
...
library.CombatColumn1 = library.LegitTab:AddColumn()
library.CombatColumn2 = library.LegitTab:AddColumn()
...
library.Rage:AddToggle({text = "Auto Sticky Detonation", flag = "rage_deto"}):AddSlider({text = "Detonation Range", flag = "rage_detorange", value = 11, min = 1, max = 30})
library.Rage:AddToggle({text = "Detonate All Stickies", flag = "rage_allstickies"})
library.Rage:AddDivider("Hack Vs Hack")
...
library.Rage:AddDivider("Hitbox Expander")
library.Rage:AddToggle({text = "Hitbox Expander", flag = "rage_Expander"})
library.Rage:AddToggle({text = "Dynamic", flag = "rage_ExpanderDynamic"})
library.Rage:AddToggle({text = "Include Buildings", flag = "rage_ExpanderBuildings"})
library.Rage:AddToggle({text = "Include Teammates", flag = "rage_Expanderteam"})
library.Rage:AddSlider({text = "Hitbox Size", flag = "rage_ExpanderSize", value = 12, min = 1, max = 30})
library.Rage:AddSlider({text = "Hitbox Transparency", flag = "rage_ExpanderTrans", value = 10, min = 0, max = 10})
--!SECTION
...
spawn(function()
    workspace.Camera.ChildAdded:Connect(function(part) 
		if isAlive(LocalPlayer) then
			if part.Name == "Arms" then
				local Model 
				Model = part:WaitForChild("CSSArms")
				...
				for i,v in pairs(part:GetChildren()) do 
					if (v:IsA("BasePart") or v:IsA("Part")) and v.Transparency ~= 1 and v.Name ~= "Flash" then 
						...
						for a,l in pairs(v:GetDescendants()) do 
							if l:IsA("BasePart") or l:IsA("Part") or l:IsA("MeshPart") then 
                    ...
                    if v:IsA("MeshPart") then
                        if v.Transparency == 1 then
                            for o, b in pairs(v:GetDescendants()) do
                                if b:IsA("MeshPart") and not b:FindFirstAncestor("Right Arm") and not b:FindFirstAncestor("Left Arm") then
                                    table.insert(WeaponT, b)
                                end
                            end
                        end
                    end
				...
				if RightArm then 
                    for i,v in pairs(RightArm:GetChildren()) do
                        if v.Name:match("Glove") or v.Name:match("Wraps") then
                            RightGlove = v
                        end
                    end
			        for i,v in pairs(RightArm:GetChildren()) do
                    ...
                    if RightGlove then
                        ...
                        GloveTexture.Value = RightGlove.TextureID
                        GloveTexture.Name = "FuckingTexture" 
                        GloveTexture.Parent = RightGlove 
                    end
					if library.flags["Local_Arm"] then
                        spawn(function()
                            local funnyconnection
                            funnyconnection = RightArm:GetPropertyChangedSignal("Transparency"):Connect(function()
                                if RightArm ~= nil then
                                    RightArm.Transparency = esp.HandChams
                                else
                                    funnyconnection:Disconnect()
                                end
                            end)
                            if RightArm.Transparency ~= esp.HandChams then
                                RightArm.Transparency = esp.HandChams
                            end
                        end)
					...
					if RightSleeve ~= nil then 
						...
						SleeveTexture.Value = RightSleeve.TextureID 
						SleeveTexture.Name = "FuckingTexture" 
				...
				if LeftArm then 
					...
					if library.flags["Local_Arm"] then
                        spawn(function()
                            local funnyconnection
                            funnyconnection = LeftArm:GetPropertyChangedSignal("Transparency"):Connect(function()
                                if LeftArm ~= nil then
                            ...
                            if LeftArm.Transparency ~= esp.HandChams then
                                LeftArm.Transparency = esp.HandChams
                            end
                        end)
					...
					local GloveTexture = INST("StringValue") 
					GloveTexture.Value = LeftGlove.TextureID
					GloveTexture.Name = "FuckingTexture" 
					...
					if LeftSleeve ~= nil then 
						local SleeveTexture = INST("StringValue") 
						SleeveTexture.Value = LeftSleeve.TextureID
						SleeveTexture.Name = "FuckingTexture" 
...
library.Players:AddToggle({text = "Building Chams", flag = "ESP_BuildingChams"}):AddColor({flag = "ESPC_BuildingChamsF", trans = 0.5, color = C3(66, 135, 245), calltrans = function(val)
    esp.BuildingChamsF = val
end}):AddColor({flag = "ESPC_BuildingChamsO", trans = 0.5, color = C3(66, 245, 176), calltrans = function(val)
    esp.BuildingChamsO = val
end})
...
library.Players:AddToggle({text = "Health Bar", flag = "ESP_HealthB"}):AddColor({flag = "ESPC_HealthB", color = C3(0, 255, 0)})
...
library.Players:AddToggle({text = "Health Bar", flag = "ESPL_HealthB"}):AddColor({flag = "ESPCL_HealthB", color = C3(0, 255, 0)})
...
library.Players:AddToggle({text = "Teammates", state = false, flag = "ESP_Team"})
...
library.Players:AddSlider({text = "HP Visibility Cap", flag = "ESP_HP", value = 90, min = 0, max = 100, suffix = "%"})
...
local wtf = {
	...
	["Pickaxe Hit"] = "rbxassetid://1347140027",
	["Tool Break"] = "rbxassetid://1067922803",
	["Vine Boom"] = "rbxassetid://7285916572",
	["Vine Boom 2"] = "rbxassetid://8080941676",
}
library.Main:AddToggle({text = "Hitsound", flag = "hitsound"}):AddList({flag = "hits",  value = "Neverlose", values = {"Skeet", "Neverlose", "Rust", "Bag", "Baimware", "Bruh", "Minecraft", "Moan", "Ding", "Osu", "Osu 2", "TF2", "TF2 Pan", "Slap", "1", "Jojo", "Vibe", "Supersmash", "Epic", "Retro", "Quake", "Hangouts", "Pickaxe Hit", "Tool Break", "Vine Boom", "Vine Boom 2"}})
library.Main:AddSlider({text = "Hitsound Volume", flag = "hitv", value = 50, min = 1, max = 100})
spawn(function()
    game.Players.LocalPlayer.ScoreFolder:GetAttributeChangedSignal("Damage"):Connect(function() 
        local val = game.Players.LocalPlayer.ScoreFolder:GetAttribute("Damage")
        ...
        if library.flags["hitsound"] then
            ...
            Sound.SoundId = wtf[library.flags["hits"]]
...
library.Main:AddList({text = "Game Tweaks",  multiselect = true, flag = "tweaks", value = "Velocity", values = {"Infinite Ammo", "Infinite Reserve Ammo", "Instant Respawn", "Auto Bunnyhop", "No Bunnyhop Speed Cap", "No Voiceline Cooldown", "Force Scream On Death", "Enable Spectator Team", "Less Team Limits", "Thirdperson Mode", "Caber Always Explodes", "No Doors", "No Fall Damage", "Always Backstab", "No Humiliation", "Always Headshot", "Allow Building Anywhere", "No Decloak When On Fire", "Debug Inventory"}})
local NoHumiliation = LocalPlayer.PlayerGui.GUI.Client.Variables.roundend:GetPropertyChangedSignal("Value"):Connect(function()
    if rawget(library.flags["tweaks"], "No Humiliation") then
        LocalPlayer.PlayerGui.GUI.Client.Variables.roundend.Value = false
    end
end)
local PrimaryAmmoChanged = LocalPlayer.PlayerGui.GUI.Client.Variables.ammocount:GetPropertyChangedSignal("Value"):Connect(function()
    if rawget(library.flags["tweaks"], "Infinite Ammo") then
        LocalPlayer.PlayerGui.GUI.Client.Variables.ammocount.Value = game.ReplicatedStorage.Weapons[LocalPlayer.PlayerGui.GUI.Client.Variables.primary.Value].Ammo.Value
    end
end)
local SecondaryAmmoChanged = LocalPlayer.PlayerGui.GUI.Client.Variables.ammocount2:GetPropertyChangedSignal("Value"):Connect(function()
    if rawget(library.flags["tweaks"], "Infinite Ammo") then
        LocalPlayer.PlayerGui.GUI.Client.Variables.ammocount2.Value = game.ReplicatedStorage.Weapons[LocalPlayer.PlayerGui.GUI.Client.Variables.secondary.Value].Ammo.Value
    end
end)
local PrimaryReserveAmmoChanged = LocalPlayer.PlayerGui.GUI.Client.Variables.primarystored:GetPropertyChangedSignal("Value"):Connect(function()
    if rawget(library.flags["tweaks"], "Infinite Reserve Ammo") then
        LocalPlayer.PlayerGui.GUI.Client.Variables.primarystored.Value = game.ReplicatedStorage.Weapons[LocalPlayer.PlayerGui.GUI.Client.Variables.primary.Value].StoredAmmo.Value
    end
end)
local SecondargyReserveAmmoChanged = LocalPlayer.PlayerGui.GUI.Client.Variables.secondarystored:GetPropertyChangedSignal("Value"):Connect(function()
    if rawget(library.flags["tweaks"], "Infinite Reserve Ammo") then
        LocalPlayer.PlayerGui.GUI.Client.Variables.secondarystored.Value = game.ReplicatedStorage.Weapons[LocalPlayer.PlayerGui.GUI.Client.Variables.secondary.Value].StoredAmmo.Value
    end
end)
library.Main:AddToggle({text = "Crithack", flag = "force_crits"}):AddBind({callback = function()
    library.options["force_crits"]:SetState(not library.flags["force_crits"])
    game.Players.LocalPlayer.PlayerGui.GUI.Client.Variables.CRITBOOSTED.Value = not library.flags["force_crits"]
end})
library.Main:AddToggle({text = "Mini-Crithack", flag = "force_minicrits"}):AddBind({callback = function()
    library.options["force_minicrits"]:SetState(not library.flags["force_minicrits"])
end})
local critboost = game.Players.LocalPlayer.PlayerGui.GUI.Client.Variables.CRITBOOSTED
critboost:GetPropertyChangedSignal("Value"):Connect(function()
	if critboost.Value ~= nil and getgenv().critboostscriptoverride ~= true and library.flags["force_crits"] then
		critboost.Value = true
	end
end)
local g = game
local w = g.Workspace
...
library.Main:AddToggle({text = "No Textures", flag = "notxt", callback = function(value)
    if value == true then
        settings().Rendering.QualityLevel = "Level01"
        for i, v in pairs(w.Map:GetDescendants()) do
            if table.find(partswithtransparency, v.ClassName) then
                v.Material = "Plastic"
                v.Reflectance = 0
        ...
        for i,v in pairs(g.ReplicatedStorage:GetDescendants()) do
...
library.Main:AddButton({text = "DEX Explorer", callback = function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/main/dex.lua"))()
end})
library.Main:AddButton({text = "Infinite Yield", callback = function()
...
library.Movement:AddToggle({text = "Walk Speed", flag = "Movement_Walk"}):AddSlider({flag = "Movement_WalkAmt", value = 18, min = 18, max = 300}):AddBind({flag = "Movement_WalkKey"})
...
library.Other:AddSlider({text = "Chat Spam Delay", flag = "spamdel", suffix = "ms", value = 500, min = 0, max = 1000})
library.Other:AddList({text = "Chat Spam Type",  flag = "spamtype", value = "Alsike", values = {"Alsike", "Bibulus"}})
library.Other:AddToggle({text = "Killsay", flag = "r_ks"})
library.Other:AddList({text = "Killsay Type",  flag = "kstype", value = "Alsike", values = {"Alsike", "Bibulus", "Toxic","Mr Sandman (Punch Out)"}})
game.Players.LocalPlayer.ScoreFolder:GetAttributeChangedSignal("Kills"):Connect(function() 
    local val = game.Players.LocalPlayer.ScoreFolder:GetAttribute("Kils")
    if val == 0 or not library.flags["r_ks"] then return end
    
    local tablething = library.flags["kstype"] == "Mr Sandman (Punch Out)" and CS["MrSandmanKS"] or CS[library.flags["kstype"] .. "KS"]
    print(tablething)
    local soontobe = tablething[math.random(1,#tablething)]
    game.ReplicatedStorage.Events.ChatMessage:FireServer(soontobe, false)
end)
...
hum.AnimationPlayed:connect(function(obj)
    if (obj.Name:match("Run") or obj.Name:match("Jump") or obj.Name:match("Stand")) and library.flags["anti_slide"] then
        obj:Stop()
    end
end)
...
local function DetonateStickies(allstickies)
	if allstickies == true then
		for i,v in pairs(workspace.Destructable:GetChildren()) do
			if v.Name == game.Players.LocalPlayer.Name.." stickybomb" then
				game:GetService("ReplicatedStorage").Events.Detonate:FireServer(v, true)
...
local function Combat(step)
    ...
    for _,v in pairs(game.Players:GetPlayers()) do
        if library.flags["rage_Expander"] and v.Name ~= plr.Name and v.Character and isTarget(v, library.flags["rage_Expanderteam"]) then
            if library.flags["rage_ExpanderDynamic"] and plr.Character and plr.Character.HumanoidRootPart and v.Character:FindFirstChild("Hitbox") and v.Character:FindFirstChild("HeadHB") then
                coroutine.resume(coroutine.create(function()
                    local ogdist = (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.Character.HumanoidRootPart.Position).Magnitude
                    local dist = ogdist + 30
                    local fv = tonumber(ogdist)
                    if dist <= library.flags["rage_ExpanderSize"] + 30 then
                        local hitbox = v.Character.Hitbox
                        local headhb = v.Character.HeadHB
                        hitbox.CanCollide = false
                        hitbox.Size = Vector3.new(fv, fv, fv)
                        hitbox.Massless = true
                        hitbox.Transparency = 1
                        headhb.CanCollide = false
                        headhb.Size = Vector3.new(fv, fv, fv)
                        ...
                        local headhb = v.Character.HeadHB
                        hitbox.CanCollide = false
                        hitbox.Size = Vector3.new(library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"])
                        hitbox.Massless = true
                        hitbox.Transparency = 1
                        headhb.CanCollide = false
                        headhb.Size = Vector3.new(library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"])
                        headhb.Massless = true
                        headhb.Transparency = 1
                    end
                end))
        ...
        if library.flags["rage_Expander"] and v.Name ~= plr.Name and v.Character and v.Team == plr.Team and v.Character:FindFirstChild("Hitbox") and v.Character:FindFirstChild("HeadHB") and library.flags["rage_Expanderteam"] == false then 
            v.Character.Hitbox.CanCollide = false
            v.Character.Hitbox.Size = v.Character.Hitbox:FindFirstChild("OriginalSize").Value or Vector2.new(2,2,2)
            v.Character.Hitbox.Massless = true
            v.Character.Hitbox.Transparency = 1
            v.Character.HeadHB.CanCollide = false
            v.Character.HeadHB.Size = v.Character.HeadHB:FindFirstChild("OriginalSize").Value or Vector2.new(2,2,2)
            v.Character.HeadHB.Massless = true
            v.Character.HeadHB.Transparency = 1
        end
    end
    for _,v in pairs(workspace:GetChildren()) do
        if library.flags["rage_ExpanderBuildings"] and library.flags["rage_Expander"] and (v.Name:match("'s Teleporter") or v.Name:match("'s Dispenser") or v.Name:match("'s Sentry")) then
            if library.flags["rage_ExpanderDynamic"] and plr.Character and plr.Character.HumanoidRootPart  then
                if v.TeamColor3.BrickColor ~= game.Players.LocalPlayer.Team.TeamColor and v:FindFirstChild("Hitbox") and v:FindFirstChild("TeamColor3") then
                    local ogdist = (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.Hitbox.Position).Magnitude
                    local dist = ogdist * 1.8
                    local fv = tonumber(ogdist)
                    if dist <= library.flags["rage_ExpanderSize"] * 1.8 then
                        v.Hitbox.Size = Vector3.new(fv, fv, fv)
                    else
                        v.Hitbox.Size = Vector3.new(library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"])
                    end
                end
            else
                if v.TeamColor3.BrickColor ~= game.Players.LocalPlayer.Team.TeamColor then
                    v.Hitbox.Size = Vector3.new(library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"], library.flags["rage_ExpanderSize"])
                end
            end
        end
	...
	for _,Player in pairs(game.Players:GetPlayers()) do
		if Player ~= plr and Player.Team ~= plr.Team and library.flags["rage_deto"] then
			for i,v in pairs(workspace.Destructable:GetChildren()) do
				if v.Name == game.Players.LocalPlayer.Name.." stickybomb" then
					if Player.Character and (v.Position - Player.Character:GetPivot().Position).Magnitude < library.flags["rage_detorange"] then
						if not library.flags["rage_allstickies"] then
							game:GetService("ReplicatedStorage").Events.Detonate:FireServer(v, true)
							game:GetService("ReplicatedStorage").Events.Detonate:FireServer(v, nil, v.CFrame)
						else
							DetonateStickies(library.flags["rage_allstickies"] or true)
						end
					end
				end
			end
    ...
    if isAlive(LocalPlayer) then
        if library.flags["anti_enabled"] and LocalPlayer.Character and LocalPlayer.Character.Humanoid then
            LocalPlayer.Character.Humanoid.AutoRotate = false 
            Pitch = library.flags["anti_pitch"] == "Up" and 1 or library.flags["anti_pitch"] == "Down" and -1 or library.flags["anti_pitch"] == "Zero" and 0 or library.flags["anti_pitch"] == "Random" and Random.new():NextNumber(-1, 1) or 0
            ...
            for i, obj in pairs(LocalPlayer.Character.Humanoid:GetPlayingAnimationTracks()) do
                if (obj.Name:match("Run") or obj.Name:match("Jump") or obj.Name:match("Stand")) and library.flags["anti_slide"] then
                    obj:Stop()
                end
            end
            ...
            game.ReplicatedStorage.UnreliableEvents.ControlTurn:FireServer(Pitch, false, nil, nil, nil) 
            ...
            game.ReplicatedStorage.UnreliableEvents.ControlTurn:FireServer(Camera.CFrame.LookVector.Y, false) 
        end
    end
end
...
local function Visuals()
    ...
    for i,v in pairs(workspace:GetChildren()) do
        if library.flags["ESP_Enabled"] and (v.Name:match("'s Sentry") or v.Name:match("'s Dispenser") or v.Name:match("'s Teleporter")) then
            local Player = game.Players:FindFirstChild(v:GetAttribute("Owner"))
            if library.flags["ESP_BuildingChams"] and Player ~= nil and isTarget(Player, library.flags["ESP_Team"]) then
                if v:FindFirstChildWhichIsA("Highlight") == nil then
                    local highlight = Instance.new("Highlight", v)
                end
                v.Highlight.FillColor = library.flags["ESPC_BuildingChamsF"]
                v.Highlight.OutlineTransparency = esp.BuildingChamsO
                v.Highlight.FillTransparency = esp.BuildingChamsF
                v.Highlight.OutlineColor = library.flags["ESPC_BuildingChamsO"]
            else
    ...
    for _,Player in pairs (Players:GetPlayers()) do
        if library.flags["ESP_Enabled"] and PLRDS[Player] and isTarget(Player, library.flags["ESP_Team"]) then
            ...
            if not OnScreen then
                ...
                if library.flags["ESP_HNum"] and Humanoid.Health <= Humanoid.MaxHealth * library.flags["ESP_HP"] / 100 then
...
function ETC(step)
    if library.flags["cheatt"] then
        library.thiswasveryconfusing.Text = library.flags["cheatname"]
    else
        library.thiswasveryconfusing.Text = "alsike"
    end

    Watermark.Text.Text = string.format("%s | v2.6 [dev] | FPS: %s | Ping: %sms", library.thiswasveryconfusing.Text, math.floor(1/step), math.floor(game:GetService('Stats').Network.ServerStatsItem["Data Ping"]:GetValue()))
...
local Speed = 0
local vadd = 0
local isvsbserver = false
if game.ReplicatedStorage.Settings.Gametype.VSB.Value then
	isvsbserver = true
end
function Misc(step)
	for i,v in pairs(game.ReplicatedStorage.Weapons:GetChildren()) do
		if v:FindFirstChild("MCBosses") and not v:GetAttribute("VSBWep") then
			v:SetAttribute("VSBWep", true)
		end
		if not v:FindFirstChild("MCBosses") and not v:GetAttribute("VSBWep") and library.flags["force_minicrits"] then
			local mcbfolder = Instance.new("Folder")
			mcbfolder.Name = "MCBosses"
			mcbfolder.Parent = v
		elseif v:FindFirstChild("MCBosses") and not v:GetAttribute("VSBWep") and not library.flags["force_minicrits"] then
			v.MCBosses:Destroy()
		end
	end
	if not isvsbserver then
		game.ReplicatedStorage.Settings.Gametype.VSB.Value = library.flags["force_minicrits"]
	end
    if isAlive(LocalPlayer) then
        ...
        if rawget(library.flags["tweaks"], "Caber Always Explodes") then
            local caber = game.ReplicatedStorage.Weapons.Caber
			if not caber:FindFirstChild("ExplodeAnyway") then
				local deez = Instance.new("Folder")
				deez.Name = "ExplodeAnyway"
				deez.Parent = caber
			end
        else
			local caber = game.ReplicatedStorage.Weapons.Caber
            if caber:FindFirstChild("ExplodeAnyway") then
				caber.ExplodeAnyway:Destroy()
			end
        end

        if rawget(library.flags["tweaks"], "Instant Respawn") then
            game.ReplicatedStorage.VIPSettings.NoRespawnTime.Value = true
        else
            game.ReplicatedStorage.VIPSettings.NoRespawnTime.Value = false
        end

        if rawget(library.flags["tweaks"], "Auto Bunnyhop") then
            game.ReplicatedStorage.VIPSettings.SpeedDemon.Value = true
        else
            game.ReplicatedStorage.VIPSettings.SpeedDemon.Value = false
        end

        if rawget(library.flags["tweaks"], "No Bunnyhop Speed Cap") then
            game.Players.LocalPlayer.Character:SetAttribute("SpeedCap", 9e9)
        end

        if rawget(library.flags["tweaks"], "No Voiceline Cooldown") then
            game.ReplicatedStorage.VIPSettings.NoVoiceCooldown.Value = true
        else
            game.ReplicatedStorage.VIPSettings.NoVoiceCooldown.Value = false
        end

        if rawget(library.flags["tweaks"], "Force Scream On Death") then
            game.ReplicatedStorage.VIPSettings.ForceDeathScreams.Value = true
        else
            game.ReplicatedStorage.VIPSettings.ForceDeathScreams.Value = false
        end

        if rawget(library.flags["tweaks"], "Enable Spectator Team") then
            game.ReplicatedStorage.VIPSettings.EnabledSpectator.Value = true
        else
            game.ReplicatedStorage.VIPSettings.EnabledSpectator.Value = false
        end

        if rawget(library.flags["tweaks"], "Less Team Limits") then
            game.ReplicatedStorage.VIPSettings.NoTeamLimits.Value = true
        else
            game.ReplicatedStorage.VIPSettings.NoTeamLimits.Value = false
        end

		if rawget(library.flags["tweaks"], "No Doors") then
			spawn(function()
				local map = workspace:FindFirstChild("Map")
				local ignore = map and map:FindFirstChild("Ignore")
				local clips = map and map:FindFirstChild("Clips")
				local geometry = map and map:FindFirstChild("Geometry")
				if not map then return end
				if workspace.Camera:FindFirstChild("Doors") then
					for _,v in pairs(workspace.Camera.Doors:GetChildren()) do
						v.CanCollide = false
				...
				if ignore then
					for i,v in pairs(ignore:GetChildren()) do
						if v.Name == "trigger_hurt" or v.Name == "GreenSpawnZone" or v.Name == "RedSpawnZone" then
							v:Destroy()
						end
					end
				end
				if clips then
					if clips:FindFirstChild("PlayerClips") then
						clips["PlayerClips"]:Destroy()
					end
					for i,v in pairs(clips:GetChildren()) do
						if v.Name:lower():match("playerclip") then
							v:Destroy()
						end
					end
				end
				if geometry and geometry:FindFirstChild("Doors") then
					geometry["Doors"]:Destroy()
				end
			end)
        end
	
		if rawget(library.flags["tweaks"], "No Fall Damage") then
            local falldmg = game.ReplicatedStorage.Events:FindFirstChild("FallDamage")
			if falldmg then
				falldmg:Destroy()
			end
        else
			local nofalldamage = not game.ReplicatedStorage.Events:FindFirstChild("FallDamage")
			if nofalldamage then
				local deez = Instance.new("RemoteEvent")
				deez.Name = "FallDamage"
				deez.Parent = game.ReplicatedStorage.Events
			end
        end

        game.ReplicatedStorage.VIPSettings.AThirdPersonMode.Value = true
        game.ReplicatedStorage.VIPSettings.NoMarksman.Value = false
...
spawn(function()
    while task.wait() do
        local chosenspam = library.flags["spamtype"] or "Alsike"
        local tablething = chosenspam .. "CS"
        if library.flags["r_chat"] then
            wait(library.flags["spamdel"] / 1000)
	        game.ReplicatedStorage.Events.ChatMessage:FireServer(CS[tablething][math.random(1, #CS[tablething])]:gsub('\"', ''), false)
        end
        
        local hasdangerousfeatures = 
            rawget(library.flags["tweaks"], "Always Backstab") or 
            rawget(library.flags["tweaks"], "Always Headshot") or 
            rawget(library.flags["tweaks"], "Allow Building Anywhere") or 
            rawget(library.flags["tweaks"], "No Decloak When On Fire") or 
            rawget(library.flags["tweaks"], "Debug Inventory")
        if hasdangerousfeatures then
            local real = game.ReplicatedStorage.Functions:FindFirstChild("UpdateSettings")
            if real then
                real:Destroy()
            end
            local updatesetting = not game.ReplicatedStorage.Functions:FindFirstChild("UpdateSettings")
			if updatesetting then
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("PowerEffects_AlwaysBackstab", rawget(library.flags["tweaks"], "Always Backstab"))
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("PowerEffects_AlwaysHeadshot", rawget(library.flags["tweaks"], "Always Headshot"))
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("BuildBypass", rawget(library.flags["tweaks"], "Allow Building Anywhere"))
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("AgentDecloakOnFire", not rawget(library.flags["tweaks"], "No Decloak When On Fire"))
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("DebugInventory", rawget(library.flags["tweaks"], "Debug Inventory"))
			end
        else
            local updatesetting = not game.ReplicatedStorage.Functions:FindFirstChild("UpdateSettings")
			if updatesetting then
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("PowerEffects_AlwaysBackstab", false)
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("PowerEffects_AlwaysHeadshot", false)
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("BuildBypass", false)
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("AgentDecloakOnFire", true)
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("DebugInventory", false)
			    game.Players.LocalPlayer.PlayerConfig:SetAttribute("NoHumiliation", false)
			    local deez = Instance.new("RemoteFunction")
				deez.Name = "UpdateSettings"
				deez.Parent = game.ReplicatedStorage.Functions
			end
		end
    end
end)
...
createEventLog("Alsike has loaded with no on-execution errors!", 5)

library:Init();

print('Alsike has loaded with no on-execution errors!')
